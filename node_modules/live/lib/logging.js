...
exports.default = function (container) {
  ...
  container.vent.on('register-plugin', function (_ref) {
    ...
    (0, _zog.log)('Registering plugin ' + _chalk2.default.bold(moduleName) + ' ' + _chalk2.default.gray(relPath));
  });
  ...
  container.vent.on('run-event', function (name, _ref3) {
    ...
    if (source) {
      ...
      (0, _zog.log)('Callback ' + prettyEvent + ' called by ' + _chalk2.default.bold(event.moduleName));
      (0, _zog.log)('  ' + link);
      // TODO: Show description?
    } else {
      //const link = makeIDELink(callInfo).gray
      var _link = '';
      (0, _zog.log)('Performing event \'' + name + '\' ' + _link);
    }
  });

  container.vent.on('add-callback', function (name, callback) {
    (0, _zog.log)('  Subscribing to ' + _chalk2.default.blue(name));
    //log('  Subscribe to', chalk.bold(name), `(${callback.moduleName})`)
  });

  container.vent.on('invalid-module', function (text) {
    (0, _zog.warn)('Invalid Live module defined in:', text);
  });
};
...
var _require = require('color-cod');

var ColorCode = _require.ColorCode;
